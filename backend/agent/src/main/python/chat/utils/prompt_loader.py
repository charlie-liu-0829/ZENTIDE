...
def load_scene_prompts():
    """Load the prompt used when the agent is answering a scene question."""
    try:
        scene_prompt_path = get_abs_path(prompts_conf["scene_prompt_path"])
    except KeyError as e:
        logger.error("[load_scene_prompts]配置中没有scene_prompt_path配置项")
        raise e
    try:
        return open(scene_prompt_path, "r", encoding="utf-8").read()
    except Exception as e:
        logger.error(f"[load_scene_prompts]解析现场问答提示词出错，{str(e)}")
        raise e
